...
function RestaurantPage({ match, map, restaurantsArray, updateMain, markers }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < restaurants.length; i++) {
      if (String(restaurants[i].place_id) === String(match.params.id)) {
    ...
    for (let i = 0; i < markers.length; i++) {
      if (markers[i].position === latLng) {
        ...
        setTimeout(animateMarker(theMarker), 1500);
      }
    }
  ...
  }, [map, markers, match.params.id, restaurants]);
  ...
  return (
    <div>
      {restaurants.map((r, index) => {
        return (
          String(match.params.id) === String(r.place_id) && (
            <div key={r.place_id} className="main boxContainer">
              <p className="breadCrumbs">
                <Link to="/">Home page</Link> >{" "}
                <span className="currentPage">{r.name}</span>
              </p>
              <button
                id="mobileAnimation"
                className="marp"
              ...
              <div className="title box">
                ...
                <p>{r.vicinity}</p>
                {r.opening_hours && (
                  <p className={r.opening_hours.open_now ? "open" : "closed"}>
                    {r.opening_hours.open_now ? "Open" : "Closed"}
                  </p>
                )}
                <Rater total={5} rating={r.rating} interactive={false} />
              ...
              <div className="box">
                <AddReviewForm restaurant={index} addNewReview={addNewReview} />
              </div>
              <div className="box">
                <h3>Reviews</h3>
                {reviews &&
                ...
                {(!reviews || parseInt(reviews.length) === 0) && (
